...
void putc(uint8_t c) {
    int i;
    // handles printing the character
    if(c == '\n' || c == '\r') {
    ...
    else if(c == '\b') { // if character is backspace
        if(screen_x > 0) {
            screen_x--;
        }
    ...
    else if(c =='\t') { // if character is tab
        for(i = 0; i < 4; i++) { // print 4 spaces
            *(uint8_t *)(video_mem + ((NUM_COLS * screen_y + screen_x) << 1)) = ' ';
            *(uint8_t *)(video_mem + ((NUM_COLS * screen_y + screen_x) << 1) + 1) = ATTRIB;
            screen_x++;
...
/* 
 * Code is from https://wiki.osdev.org/Text_Mode_Cursor 
...
void get_screen_x_y(int *x, int *y) {
    *x = screen_x;
    *y = screen_y;
}
